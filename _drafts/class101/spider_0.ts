...
import { sleepAsync, existsFile, saveFile, loadFile, loadJson, saveJson, Chrome } from 'jnj-utils';
...
// // 강의 정보 추출 및 수업자료 다운로드(lecture)
// const addClassLecture = async (lecture: any, lectures: any[],classId: string, chrome: any) => {
...
//     const xpath = `//button[contains(@class, "css-1hvtp3b")]//span[contains(text(), concat('', '${escapedTitle}', ''))]//ancestor::button`;

//     const titleButtons = await chrome.driver.findElements(By.xpath(xpath));

//     // 같은 제목의 버튼이 여러 개인 경우, 현재 강의가 몇 번째인지 확인
//     const titleIndex = lectures.filter((l) => l.title === lecture.title).findIndex((l) => l.sn === lecture.sn);

//     const button = titleButtons[titleIndex];

//     if (!button) {
//       console.error(`강의 버튼을 찾을 수 없습니다: ${lecture.title} (${titleIndex + 1}번째)`);
//       return;
//     }
...
const saveClassLectures = async (classId: string, type: string = 'update') => {
  ...
  try {
    ...
    for (const lecture of lectures) {
      ...
      // console.log(`saveClassLectures: [${lecture.sn}] ${lecture.title} `);
