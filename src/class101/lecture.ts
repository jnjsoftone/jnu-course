 ...
 */
const extractLectures = (html: string, selectors: any = {}): any[] => {
  selectors = {
 ...
 */
const downloadVttFiles = async (htmlContent: string, outputDir: string = 'subtitles'): Promise<any[]> => {
    ...
    if (src && src.endsWith('.vtt')) {
      try {
        ...
        if (error instanceof Error) {
          ...
          console.error(`다운로드 실패 (${src}): ${error.toString()}`);
        }
      }
    }
  }
 ...
 */
const saveClassLectures = async (classId: string, type: string = 'init', lectureSns: any[] = []): Promise<void> => {
  let chrome: any = null;
  let lectureIds: any[] = [];
  try {
    const url = `https://class101.net/ko/classes/${classId}`;
 ...
 * @return void
 */
const processClasses = async (myclasses: any[] = [], type: string = 'init'): Promise<void> => {
 ...
 * @return void
 */
const processClassLectures = async (myclasses: any[] = [], type: string = 'init'): Promise<void> => {
  if (myclasses.length === 0) {
    return;
  }
  ...
  for (const myclass of myclasses) {
    try {
      const lectureSns = myclass.lectures.map((l: any) => l.sn);
